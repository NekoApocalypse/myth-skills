# Stage 2: Deep Observation (Depth, Logic & Proxies)

## Objective
Take the hottest themes and events identified in Stage 1 and search for top-tier, in-depth, contrarian institutional research. Extract the underlying causal frameworks and define hard, falsifiable metrics.

## Execution Steps

1. **Contrarian & Deep Search:**
   - AI training data defaults to retail thinking. Actively search for outlier, contrarian, and high-depth analyses to separate your findings from common retail noise.
   - Query specifically for deep-dive reports or contrarian takes on the themes found in Stage 1 (e.g., `"[Theme] 深度 研报"`, `"[Theme] 核心矛盾"`, `"[Theme] 证伪"`). Look for how *top-tier analysts* view the events.

2. **Extract the Core Logic:**
   - Identify the causal frameworks and hidden logic. 
   - What is the unique perspective or structural truth that differs from public consensus or common sense? 
   - Why is the smart money acting the way it is?

3. **Identify the Core Proxy (核心观测指标):**
   - A narrative must be backed by data. Find the *Core Proxy*.
   - This must be a specific, observable fact, metric, or data point (e.g., a specific commodity price spread, a high-frequency supply chain metric, a specific policy wording).
   - This proxy must possess the "hardness" to either **confirm** or **falsify** the narrative.

## Output for Stage 2
Compile the core logic, contrarian views, and explicit Core Proxies. Use this internally to feed into Stage 3.